...
##compute $\Delta$ values using objects created above...
...
```{r}
expt2 <- FPHcalc %>% dplyr::filter(expt==" 2A") %>% dplyr::select(FPH_EtOH, rouse, daysonhops,brew_date, pounds_bbl,form_of_hops,ABW.NHgroup,dayofaddition)
...
Using this model, the relationship between hop-induced ethanol production (FPH_EtOH) and hop contact time (daysonhops) can be expressed as:
