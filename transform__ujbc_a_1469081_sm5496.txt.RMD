---
title: "The Freshening Power of the Hop"
...
# descriptive and exploratory analysis using dplyr
...
#earliest sample any each day of week
mydata %>% 
  mutate(dayofweek = format(testtime, "%A")) %>% 
  group_by(dayofweek) %>% 
  filter(Test.time==min(Test.time)) %>%
  ungroup %>% 
...
unique_rows <- !duplicated(df[c("Hop_type","EXPTnew.x","temperature")])  ## identify unique combinations
